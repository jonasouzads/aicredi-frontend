...
export function CreateAgentModal({ onClose }: CreateAgentModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Agent criado!', `O agent "${formData.name}" foi criado com sucesso.`);
      onClose();
    } catch (error: any) {
      toast.error('Erro ao criar agent', error.message || 'Não foi possível criar o agent.');
    } finally {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between mb-6">
          <h2 id="create-agent-title" className="text-title text-text-primary">Criar Novo Agent</h2>
          <button
            onClick={onClose}
